- [ ] 1. Bootstrap repository structure and shared Terraform foundations
  ...
  - [ ] 1.4 Run `terraform fmt -check` and `terraform validate` in `infra/`.
...
- [ ] 2. Implement infrastructure modules for runtime services
  ...
  - [ ] 2.5 Wire all modules in root `infra/main.tf` and expose function URLs/secrets references through outputs.
  - [ ] 2.6 Run `terraform fmt -check` and `terraform validate` in `infra/`.
